      ...
      try:
         ...
         for json_obj in sorted_path: ## Using reversed(list) retrieves entries chronologically from most recent to oldest
            ...
            try:
               ...
               new_inc_period_days = -1

               if filed and end:
                  try:  
                     filed_period_days = calculate_period_days(end, filed)
                     if filed_period_days >= 80:
                        continue
               ...
               if start and end:
                  try:  
                     new_inc_period_days = calculate_period_days(start, end)
                  except Exception as e:
                        print(e)

               # if new_inc_period_days < 120 and frame:
               #    continue

               if new_inc_period_days > 100 and json_obj.get('fp')[0] == "Q":
                  continue
               
               if new_inc_period_days > 300 and json_obj.get('fp')[0] != "F":
                  continue
               ...
               new_inc=IncomeStatement(
                  ...
                  frame = frame,
                  period_days = new_inc_period_days
               )

               accns.append(new_inc.accn)
...
def set_operating_income(co_inc_key: str):
   for k, db_attr in EXPENSE_KEYS.items():
      try:
         path_usd = path.get(k).get('units',{}).get('USD',[])
         if not path_usd:
            continue
         ...
         for json_obj in reversed_path:
            ...
            if k == 'OperatingIncomeLoss':
               inc.total_expenses_net_of_tax = inc.total_revenue - inc.operating_income
               print(inc.total_expenses_net_of_tax)
      except Exception as e:
         # print(f'{k} - {e} - {company.ticker}')
         continue
...
if __name__ == '__main__':
   ...
   with app.app_context():
      ...
      # companies = [Company.query.filter(Company.id == 11).first()]
